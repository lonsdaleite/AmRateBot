    ...
    if page.status_code == 200:
        ...
        for rate in rates:
            ...
            if pos.text in ["USD", "EUR", "RUR"]:
                ...
                add_rate(all_rates, currency, "cash", "amd", "cash", "sas", float(pos.text), "to")
                pos = pos.findNext("span", class_="exchange-table__cell-content")
                add_rate(all_rates, "amd", "cash", currency, "cash", "sas", float(pos.text), "from")
            if total == 3:
                break
    else:
        print("ERROR: SAS status code: " + str(page.status_code))