...
def get_visa_rate(url="https://usa.visa.com/cmsapi/fx/rates?amount=1", from_curr="AMD", to_curr="RSD",
    ...
    full_rate = get_json_rate(dt, url, from_curr, to_curr, convert_fee_prc)
    # print(full_rate)
...
def add_visa(all_rates=None):
    # ArdshinBank
    ardshin_convert_fee_prc = 0
    ardshin_cash_fee = 0.015
    ardshin_c2c_fee = 0.005

    ardshin_amd_rsd_rate = get_visa_rate(convert_fee_prc=ardshin_convert_fee_prc, from_curr="AMD", to_curr="RSD")
    ardshin_eur_rsd_rate = get_visa_rate(convert_fee_prc=ardshin_convert_fee_prc, from_curr="EUR", to_curr="RSD")
    ardshin_amd_eur_rate = get_visa_rate(convert_fee_prc=ardshin_convert_fee_prc, from_curr="AMD", to_curr="EUR")
    ...
    # Unibank
    unibank_convert_fee_prc = 2
    unibank_cash_fee = 0.013

    unibank_amd_rsd_rate = get_visa_rate(convert_fee_prc=unibank_convert_fee_prc, from_curr="AMD", to_curr="RSD")
    unibank_usd_rsd_rate = get_visa_rate(convert_fee_prc=unibank_convert_fee_prc, from_curr="USD", to_curr="RSD")
